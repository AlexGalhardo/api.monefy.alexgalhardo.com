...
export default class RabbitMQ {
    ...
    public async sendMessageForgetPassword(message: string) {
        console.log("Entrou sendMessageForgetPasswordToken com message: ", message);
        try {
            this.channel = await this.connection!.createChannel();
            await this.channel.assertQueue("auth_forget_password_queue", {
                durable: true,
            });

            this.channel.sendToQueue("auth_forget_password_queue", Buffer.from(JSON.stringify(message)), {
                persistent: true,
            });
        } catch (error) {
            console.log("Error sendMessageForgetPassword: ", error);
        }
    }
    ...
    public async consumeMessages() {
        try {
            ...
            await this.channel.assertQueue("auth_forget_password_queue", { durable: true });
            await this.channel.consume(
                "user_signup_queue",
                async (message) => {
                    if (message) {
