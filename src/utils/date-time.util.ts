export default class DateTime {
	static timestampToGetNow(timestamp: number) {
		const date = new Date(timestamp * 1000).toLocaleDateString(Bun.env.LOCALE_DATE_TIME);
		const time = new Date(timestamp * 1000).toLocaleTimeString(Bun.env.LOCALE_DATE_TIME);
		return `${date} ${time}`;
	}

	static getNow() {
		const date = new Date().toLocaleDateString(Bun.env.LOCALE_DATE_TIME);
		const time = new Date().toLocaleTimeString(Bun.env.LOCALE_DATE_TIME);
		return `${date} ${time}`;
	}
